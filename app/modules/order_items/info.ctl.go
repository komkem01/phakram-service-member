...
type InfoOrderItemControllerRequestUri struct {
	ID string `uri:"id"`
}

type InfoOrderItemControllerResponses struct {
...
func (c *Controller) InfoController(ctx *gin.Context) {
	...
	var req InfoOrderItemControllerRequestUri
	if err := ctx.ShouldBindUri(&req); err != nil {
		log.With(slog.Any(`body`, req)).Errf(`internal: %s`, err)
		base.BadRequest(ctx, i18n.BadRequest, nil)
		return
	}
	span.AddEvent(`order_items.ctl.info.request`)
	...
	span.AddEvent(`order_items.ctl.info.callsvc`)

	var resp InfoOrderItemControllerResponses
...
func (c *Controller) OrderItemsInfo(ctx *gin.Context) {
