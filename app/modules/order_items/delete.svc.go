...
func (s *Service) DeleteService(ctx context.Context, id uuid.UUID, memberID uuid.UUID) error {
	...
	span.AddEvent(`order_items.svc.delete.start`)

	if err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		if _, err := tx.NewDelete().Model(&ent.OrderItemEntity{}).Where("id = ?", id).Exec(ctx); err != nil {
			return err
		}
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionDetail: "Deleted order item " + id.String(),
			CreatedAt:    now,
			UpdatedAt:    now,
		}
	...
	span.AddEvent(`order_items.svc.delete.success`)
	return nil
}
