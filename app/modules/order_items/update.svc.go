...
type UpdateOrderItemService struct {
...
func (s *Service) UpdateService(ctx context.Context, id uuid.UUID, req *UpdateOrderItemService) error {
	span, log := utils.LogSpanFromContext(ctx)
	span.AddEvent(`order_items.svc.update.start`)

	data, err := s.db.GetOrderItemByID(ctx, id)
	if err != nil {
	...
	if err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionDetail: "Updated order item " + data.ID.String(),
			CreatedAt:    now,
			UpdatedAt:    now,
		}
	...
	span.AddEvent(`order_items.svc.update.success`)
	return nil
}
