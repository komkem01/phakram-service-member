...
func (s *Service) DeleteService(ctx context.Context, id uuid.UUID) error {
	...
	err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		if _, err := tx.NewDelete().Model(&ent.PrefixEntity{}).Where("id = ?", id).Exec(ctx); err != nil {
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionType:   "delete_prefix",
			ActionID:     id,
			ActionBy:     nil,
			Status:       ent.StatusAuditSuccesses,
			ActionDetail: "Deleted prefix with ID " + id.String(),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
	...
	if err != nil {
		span.AddEvent(`prefixes.svc.delete.failed`)
		failLog := &ent.AuditLogEntity{
			...
			ActionDetail: fmt.Sprintf("Delete prefix failed: %v", err),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
