...
func (s *Service) DeleteService(ctx context.Context, id uuid.UUID) error {
	...
	span.AddEvent(`categories.svc.delete.start`)

	err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		if _, err := tx.NewDelete().Model(&ent.CategoryEntity{}).Where("id = ?", id).Exec(ctx); err != nil {
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionDetail: "Deleted category with ID " + id.String(),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
	...
	if err != nil {
		span.AddEvent(`categories.svc.delete.failed`)
		failLog := &ent.AuditLogEntity{
			...
			ActionDetail: fmt.Sprintf("Delete category failed: %v", err),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
	...
	span.AddEvent(`categories.svc.delete.success`)
	return nil
}
