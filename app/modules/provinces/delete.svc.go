...
func (s *Service) DeleteService(ctx context.Context, id uuid.UUID) error {
	...
	err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		if _, err := tx.NewDelete().Model(&ent.ProvinceEntity{}).Where("id = ?", id).Exec(ctx); err != nil {
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionDetail: "Deleted province with ID " + id.String(),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
	...
	if err != nil {
		span.AddEvent(`provinces.svc.delete.failed`)
		failLog := &ent.AuditLogEntity{
			...
			ActionDetail: fmt.Sprintf("Delete province failed: %v", err),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
