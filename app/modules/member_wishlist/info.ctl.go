...
type InfoMemberWishlistControllerRequestUri struct {
	ID string `uri:"id"`
}

type InfoMemberWishlistControllerResponses struct {
...
func (c *Controller) InfoController(ctx *gin.Context) {
	...
	var req InfoMemberWishlistControllerRequestUri
	if err := ctx.ShouldBindUri(&req); err != nil {
		log.With(slog.Any(`body`, req)).Errf(`internal: %s`, err)
		base.BadRequest(ctx, i18n.BadRequest, nil)
		return
	}
	span.AddEvent(`member_wishlist.ctl.info.request`)
	...
	span.AddEvent(`member_wishlist.ctl.info.callsvc`)

	var resp InfoMemberWishlistControllerResponses
...
func (c *Controller) MemberWishlistInfo(ctx *gin.Context) {
