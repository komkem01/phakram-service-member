...
type InfoOrderControllerRequestUri struct {
	ID string `uri:"id"`
}

type InfoOrderControllerResponses struct {
...
func (c *Controller) InfoController(ctx *gin.Context) {
	...
	var req InfoOrderControllerRequestUri
	if err := ctx.ShouldBindUri(&req); err != nil {
		log.With(slog.Any(`body`, req)).Errf(`internal: %s`, err)
		base.BadRequest(ctx, i18n.BadRequest, nil)
		return
	}
	span.AddEvent(`orders.ctl.info.request`)
	...
	span.AddEvent(`orders.ctl.info.callsvc`)

	var resp InfoOrderControllerResponses
...
func (c *Controller) OrdersInfo(ctx *gin.Context) {
