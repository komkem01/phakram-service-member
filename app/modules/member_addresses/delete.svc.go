...
func (s *Service) DeleteService(ctx context.Context, id uuid.UUID, memberID uuid.UUID) error {
	...
	span.AddEvent(`member_addresses.svc.delete.start`)

	if err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		if _, err := tx.NewDelete().Model(&ent.MemberAddressEntity{}).Where("id = ?", id).Exec(ctx); err != nil {
			return err
		}
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionDetail: "Deleted member address " + id.String(),
			CreatedAt:    now,
			UpdatedAt:    now,
		}
	...
	span.AddEvent(`member_addresses.svc.delete.success`)
	return nil
}
