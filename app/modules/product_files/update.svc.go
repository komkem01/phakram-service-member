...
type UpdateProductFileService struct {
	ProductID uuid.UUID `json:"product_id"`
	FileID    uuid.UUID `json:"file_id"`
	MemberID  uuid.UUID `json:"member_id"`
}

func (s *Service) UpdateService(ctx context.Context, id uuid.UUID, req *UpdateProductFileService) error {
	span, log := utils.LogSpanFromContext(ctx)
	span.AddEvent(`product_files.svc.update.start`)
	...
	if err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionDetail: "Updated product file " + data.ID.String(),
			CreatedAt:    now,
			UpdatedAt:    now,
		}
	...
	span.AddEvent(`product_files.svc.update.success`)
	return nil
}
