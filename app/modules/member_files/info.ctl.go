...
type InfoMemberFileControllerRequestUri struct {
	ID string `uri:"id"`
}

type InfoMemberFileControllerResponses struct {
...
func (c *Controller) InfoController(ctx *gin.Context) {
	...
	var req InfoMemberFileControllerRequestUri
	if err := ctx.ShouldBindUri(&req); err != nil {
		log.With(slog.Any(`body`, req)).Errf(`internal: %s`, err)
		base.BadRequest(ctx, i18n.BadRequest, nil)
		return
	}
	span.AddEvent(`member_files.ctl.info.request`)
	...
	span.AddEvent(`member_files.ctl.info.callsvc`)

	var resp InfoMemberFileControllerResponses
...
func (c *Controller) MemberFilesInfo(ctx *gin.Context) {
