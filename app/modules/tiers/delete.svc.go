...
func (s *Service) DeleteService(ctx context.Context, id uuid.UUID) error {
	...
	err := s.bunDB.DB().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		if _, err := tx.NewDelete().Model(&ent.TierEntity{}).Where("id = ?", id).Exec(ctx); err != nil {
		...
		auditLog := &ent.AuditLogEntity{
			...
			ActionType:   "delete_tier",
			ActionID:     id,
			ActionBy:     nil,
			Status:       ent.StatusAuditSuccesses,
			ActionDetail: "Deleted tier with ID " + id.String(),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
	...
	if err != nil {
		span.AddEvent(`tiers.svc.delete.failed`)
		failLog := &ent.AuditLogEntity{
			...
			ActionDetail: fmt.Sprintf("Delete tier failed: %v", err),
			CreatedAt:    time.Now(),
			UpdatedAt:    time.Now(),
		}
