...
type InfoBankControllerRequestUri struct {
	ID string `uri:"id"`
}

type InfoBankControllerResponses struct {
...
func (c *Controller) InfoController(ctx *gin.Context) {
	...
	var req InfoBankControllerRequestUri
	if err := ctx.ShouldBindUri(&req); err != nil {
		log.With(slog.Any(`body`, req)).Errf(`internal: %s`, err)
		base.BadRequest(ctx, i18n.BadRequest, nil)
		return
	}
	span.AddEvent(`banks.ctl.info.request`)
	...
	span.AddEvent(`banks.ctl.info.callsvc`)

	var resp InfoBankControllerResponses
...
func (c *Controller) BanksInfo(ctx *gin.Context) {
