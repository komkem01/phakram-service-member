...
type DeletePaymentControllerRequest struct {
	ID string `uri:"id"`
}
...
func (c *Controller) DeleteController(ctx *gin.Context) {
	...
	var req DeletePaymentControllerRequest
	if err := ctx.ShouldBindUri(&req); err != nil {
		log.With(slog.Any(`body`, req)).Errf(`internal: %s`, err)
		base.BadRequest(ctx, i18n.BadRequest, nil)
		return
	}
	span.AddEvent(`payments.ctl.delete.request`)
	...
	span.AddEvent(`payments.ctl.delete.callsvc`)
