...
func (s *Service) CreateMemberWishlist(ctx context.Context, memberWishlist *ent.MemberWishlistEntity) error {
	_, err := s.db.NewInsert().
		Model(memberWishlist).
		Exec(ctx)
	return err
}
...
func (s *Service) UpdateMemberWishlist(ctx context.Context, memberWishlist *ent.MemberWishlistEntity) error {
	_, err := s.db.NewUpdate().
		Model(memberWishlist).
		Where("id = ?", memberWishlist.ID).
		Exec(ctx)
	return err
}

func (s *Service) DeleteMemberWishlist(ctx context.Context, memberWishlistID uuid.UUID) error {
	_, err := s.db.NewDelete().
		Model(&ent.MemberWishlistEntity{}).
		Where("id = ?", memberWishlistID).
		Exec(ctx)
	return err
}
