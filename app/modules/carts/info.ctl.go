...
type InfoCartControllerRequestUri struct {
	ID string `uri:"id"`
}

type InfoCartControllerResponses struct {
...
func (c *Controller) InfoController(ctx *gin.Context) {
	...
	var req InfoCartControllerRequestUri
	if err := ctx.ShouldBindUri(&req); err != nil {
		log.With(slog.Any(`body`, req)).Errf(`internal: %s`, err)
		base.BadRequest(ctx, i18n.BadRequest, nil)
		return
	}
	span.AddEvent(`carts.ctl.info.request`)
	...
	span.AddEvent(`carts.ctl.info.callsvc`)

	var resp InfoCartControllerResponses
...
func (c *Controller) CartsInfo(ctx *gin.Context) {
